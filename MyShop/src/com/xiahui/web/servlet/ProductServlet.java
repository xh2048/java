...
import com.xiahui.domain.Cart;
import com.xiahui.domain.CartItem;
import com.xiahui.domain.Category;
import com.xiahui.domain.Order;
import com.xiahui.domain.OrderItem;
import com.xiahui.domain.PageBean;
import com.xiahui.domain.Product;
import com.xiahui.domain.User;
import com.xiahui.service.ProductService;
import com.xiahui.utils.CommonsUtils;
import com.xiahui.utils.JedisPoolUtils;
import com.xiahui.utils.PaymentUtil;
...
/*	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			for(Order order : orderList){
				...
				for(Map<String,Object> map: mapList){
					...
					try {
						...
						//从map中取出pimage pname shop_price 封装到Product 中
	...
	public void confirmOrder(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//只接入一个接口，这个节接口已经集成所有的银行接口了，这个接口是第三方支付平台提供的
		//接入的是易宝支付
		//获得支付必须基本数据
	...
	public void submitOrder(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//转换new Date()格式，直接使用mysql5.6与Mysql-connector-java版本不匹配，要报错
		
		SimpleDateFormat temp=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");  
        String date=temp.format(new Date());  
        Date date2 = null;
		try {
			date2 = temp.parse(date);
		...
		for(Map.Entry<String, CartItem> entry : cartItems.entrySet()){
			...
			//2)private int count;//订单项内商品的购物数量
			orderItem.setCount(cartItem.getBuyNum());
			//3)private double subtotal;//订单项内部的商品
	...
	public void categoryList(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//先从缓存中查询categoryList，如果有直接使用，没有在从数据库中查询  存在缓存中
	...
	public void productInfo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//获得客户端懈怠cookie---获得名字是pids的cookie
		String pids = pid;
